...
function fnPresentBox() {
	...
	if (document.getElementById('button_fp_ng') != null) {
		...
		divTag.style["z-index"] = 1000; 
		divTag.style.position = "relative"; 
		divTag.innerHTML = '<button class="sexybutton sexysimple sexyblue" onmousedown="alert($(\'#\'+document.getElementById(\'presents\').childNodes[0].id)).data(\'box\');alert($(\'#\'+document.getElementById(\'presents\').childNodes[0].id).data(\'box\').boxed_id);//for (var i=0;i<$(\'#presents\').childNodes.length;i++){onReceive(null, $(\'#presents\').childNodes[i].data.box);}"><span class="download2">Receive All</span></button>'; 
