...
function fnProfileFixTabs() {
	...
	towerTrapSelectorHTML += '<option ' + (fnTowerTrap()==4003?'selected':'') + ' value="4003">Mission 1-2\'s loot</option>';
	...
	missionProcess = function() {
		$.ajax_ex(false, '/en/'+platform+'/tower/process', {'area_id'    : areaMaster.area_id,'mission_id' : mission.last_mission_id, api : 'json','full_power':true, '__hash': ('' + (new Date()).getTime())}, function(result) {
			...
			if (result.payload.process.cage) {
				...
				if (fnTowerTrap() == 4002 || fnTowerTrap() == 4003) {
					clearInterval(missionInterval);
							...
							getRedFlower = function() {
								if (parseInt(player.power,10)) {
									...
									$.ajax_ex(false, '/en/'+platform+'/mission/process?area_id=1&mission=0&confirm_id='+red_flower_confirm_id, {}, function(result2) {
										...
										if (result2.payload.event && result2.payload.event.treasure && parseInt(result2.payload.event.treasure.item_id,10)==fnTowerTrap()) {
											red_flower_count++;
...
function fnAdventureCheckItem() {
	if (adventureItemArray.length) {
		$.ajax_ex(true, '/en/'+platform+'/adventure/ajaxGetTreasureInfo', {item_id:adventureItemArray[0]},function(data) {
			if (typeof(data.payload) != 'undefined' && (data.payload.limited_flag == true || data.payload.area_id != 10001)) {
...
function fnGetMissionLoot(pArea, pMission) {
	...
	$.ajax_ex(false, '/en/'+platform+'/mission/process?area_id=' + pArea + '&mission='+pMission+'&confirm_id='+loot_confirm_id, {}, function(result2) {
		window.parent.postMessage((new Date()).getTime(), "*");
		if (parseInt(result2.status,10)==4) {
			fnDrinkMyEP('');
			player.power = player.power_max;
		}
...
function fnSuperRaidBattle() {
	...
	missionInterval = setInterval(fnSuperRaidAjaxActBattle,Math.max(3000,fnGetGrindingSpeed()));
}
